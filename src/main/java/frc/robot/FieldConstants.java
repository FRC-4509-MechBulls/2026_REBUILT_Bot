...
public class FieldConstants {
  public static final AprilTagFieldLayout fieldType = AprilTagFieldLayout.loadField(AprilTagFields.kDefaultField);
  ...
  public static class Hub {
    ...
    static {
        Optional<Pose3d> tagPose = AprilTagLayoutType.OFFICIAL.getLayout().getTagPose(26);
        
        // Default values in case the pose is missing
        double defaultX = 0.0; // Adjust these defaults as necessary
        double defaultY = fieldWidth / 2.0;
        
        // Set topCenterPoint
        topCenterPoint = tagPose.map(pose -> new Translation3d(
                pose.getX() + width / 2.0, 
                defaultY, 
                height
        )).orElse(new Translation3d(defaultX + width / 2.0, defaultY, height));

        // Set innerCenterPoint
        innerCenterPoint = tagPose.map(pose -> new Translation3d(
                pose.getX() + width / 2.0, 
                defaultY, 
                innerHeight
        )).orElse(new Translation3d(defaultX + width / 2.0, defaultY, innerHeight));
    }
